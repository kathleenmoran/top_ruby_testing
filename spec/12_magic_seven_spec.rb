...
describe MagicSeven do
  ...
  subject(:game12) { described_class.new(12) }
  describe '#subtract_random_number' do
    context 'when the random number 12 and the given number is 34' do
      it 'returns 22' do
        result = game12.subtract_random_number(34)
  ...
  describe '#play' do
    context 'when the random number is 42' do
      subject(:game42) { described_class.new(42) }
    ...
    context 'when the random number is 0' do
      subject(:game0) { described_class.new(0) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game0.play
        expect(result).to eq(7)
    ...
    context 'when the random number is -10' do
      # remove the 'x' before running this test
      subject(:game_neg10) { described_class.new(-10) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = game_neg10.play
        expect(result).to eq(7)
      end
    end
  end
