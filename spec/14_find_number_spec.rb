...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 0 and max is 9' do
      it 'returns 4' do
        guess = 4
        expect(game_guessing.make_guess).to eq(guess)
      end
    end
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_guessing7) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        guess = 7
        expect(game_guessing7.make_guess).to eq(guess)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_guessing8) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        guess = 8
        expect(game_guessing8.make_guess).to eq(guess)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_guessing1) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        guess = 1
        expect(game_guessing1.make_guess).to eq(guess)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_guessing3) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        guess = 3
        expect(game_guessing3.make_guess).to eq(guess)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_game_over) { double('random_number', value: 8) }
      subject(:game_over) { described_class.new(0, 9, number_game_over, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_game_not_over) { double('random_number', value: 8) }
      subject(:game_not_over) { described_class.new(0, 9, number_game_not_over, 4) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:same_as_max_guess_game) { described_class.new(5, 8, number_range, 7) }

      it 'updates min to the same value as max' do
        expect { same_as_max_guess_game.update_range }.to change { same_as_max_guess_game.min }.to(same_as_max_guess_game.max)
      end

      it 'does not update max' do
        expect { same_as_max_guess_game.update_range }.not_to change { same_as_max_guess_game.max }.from(8)
      end
    end

    context 'when the guess is 7, min=7, and max=8' do
      let(:number_range7) { double('random_number', value: 5) }
      subject(:same_as_min_guess_game) { described_class.new(5, 6, number_range7, 6) }

      it 'updates max to 9' do
        expect { same_as_min_guess_game.update_range }.to change { same_as_min_guess_game.max }.to(same_as_min_guess_game.min)
      end

      it 'does not update min' do
        expect { same_as_min_guess_game.update_range }.not_to change { same_as_min_guess_game.min }.from(5)
      end
    end
  end
end
